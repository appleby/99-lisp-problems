...
(defun files-matching-pattern (pattern &key depends (excludes nil))
  (loop for path in (directory pattern)
     unless (member (pathname-name path) excludes :test #'string=)
     collect (list :file (pathname-name path) :depends-on (remove (pathname-name path) depends :test #'string=))))
