...
(lisp-unit:define-test p40-goldbach-known-inputs
  (lisp-unit:assert-equal '(5 23) (p40-goldbach 28)))

(lisp-unit:define-test p40-goldbach-test
  (loop for i from 4 upto 1000 by 2
     do (let ((result (p40-goldbach i)))
	  (lisp-unit:assert-true (every #'p31-is-prime result))
	  (lisp-unit:assert-eql i (apply #'+ result)))))
